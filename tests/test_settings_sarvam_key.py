...
def test_sarvam_key_uppercase_enables_direct_mode():
    """With SARVAM_API_KEY set, USE_SARVAM_DIRECT must be True and
    LLM_BASE_URL must point to Sarvam (not HuggingFace)."""
    settings = _reload_settings({"SARVAM_API_KEY": "test-key-12345"})
...
def test_missing_sarvam_key_falls_back_to_hf():
    """Without SARVAM_API_KEY, USE_SARVAM_DIRECT must be False and
    LLM_BASE_URL must point to HuggingFace."""
    settings = _reload_settings({"SARVAM_API_KEY": None})
